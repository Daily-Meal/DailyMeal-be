...
import { getMyBoards, getMyLikeBoards } from "../services/myPageService";
...
export async function getMyBoardsController(req: Request, res: Response) {
  try {
    ...
    const { boards, total } = await getMyBoards(userId, limit, offset);
...
export async function getMyLikeBoardsController(req: Request, res: Response) {
  try {
    const userId = req.userId;
    if (!userId) {
      return res
    ...
    const { boards, total } = await getMyLikeBoards(userId, limit, offset);
    ...
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
      isSuccess: false,
      message: "Failed to fetch like boards",
    });
  }
}
